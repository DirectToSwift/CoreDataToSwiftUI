 ...
 */
public protocol D2SLogger {
  
  func primaryLog(_ logLevel: D2SLoggerLogLevel, _ msgfunc: () -> String,
                  _ values: [ Any? ] )
  
}

public extension D2SLogger { // Actual logging funcs
...
public enum D2SLoggerLogLevel : Int8 { // cannot nest types in generics
  case Error
  case Warn
  case Log
  case Info
 ...
 */
public var globalD2SLogger : D2SLogger = {
  #if DEBUG
    let defaultLevel = D2SLoggerLogLevel.Log
  #else
    let defaultLevel = D2SLoggerLogLevel.Error
  #endif
  let logEnv = ProcessInfo.processInfo.environment["D2S_LOGLEVEL"]?
                 .lowercased()
               ?? ""
  let level : D2SLoggerLogLevel
  ...
  return D2SPrintLogger(level: level)
}()
...
fileprivate let stderrLogLevel : D2SLoggerLogLevel = .Error

public struct D2SPrintLogger : D2SLogger {
  // public, maybe useful for ZeeQL users as well.
  
  let logLevel : D2SLoggerLogLevel
  
  public init(level: D2SLoggerLogLevel = .Error) {
    logLevel = level
  }
  
  public func primaryLog(_ logLevel : D2SLoggerLogLevel,
                         _ msgfunc  : () -> String,
                         _ values   : [ Any? ] )
  {
...
fileprivate extension D2SLoggerLogLevel {
